...
class BlockFragment : BaseFragment(R.layout.fragment_block), InterfaceViewBlock {

    companion object {
        const val TAG = "BlockFragment"
    }
    ...
    @Inject
    lateinit var interactor: InterfaceInteractorBlock<InterfaceViewBlock>
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        setToolbar(toolbar, true, R.string.block, 0)
    ...
    override fun successResult(result: Boolean, filter: Boolean) {
        ...
        if (result) {
            ...
            if (filter) {
                viewProgress.show(); viewNotHave.hide()
            } else {
                viewProgress.hide()
                viewNotHave.show()
                txtNotHave.text = getString(R.string.not_block_contact_yet)
            }
        }
    }
    ...
    override fun failedResult(throwable: Throwable) {
        ...
        list.hide()
        viewFailed.show()
        txtFailed.text = "${getString(R.string.failed_block)}, ${throwable.message}"
    }

    override fun setRecyclerAdapter(recyclerAdapter: BlockRecyclerAdapter) {
    ...
    private fun blockNumber() {
        ...
        builder.setTitle("Enter Number To Block")
        ...
        builder.setPositiveButton("Block") { _, _ ->
            ...
            interactor.blockNumber(userInput)
        }
    ...
    private fun itemSelected(key: String?) {
        if (!key.isNullOrEmpty()) {
            ...
            if (dataList.isNotEmpty()) toolbar.setTitle =
                "${dataList.size} ${getString(R.string.selected)}"
